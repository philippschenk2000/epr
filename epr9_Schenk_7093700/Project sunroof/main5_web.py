...
@app.route("/")
def getdata():
    laender = ['Austria', 'Belgium', 'Bulgaria', 'Switzerland', 'Cyprus', 'Czechia', 'Germany', 'Denmark', 'Estonia',
               'Greece', 'Spain', 'Finland', 'France', 'Hungary', 'Croatia', 'Ireland', 'Italy', 'Luxembourg',
               'Liechtenstein', 'Latvia', 'Lithuania', 'Netherlands', 'Norway', 'Poland', 'Portugal', 'Romania',
    ...
    for i in range(0, 1):
        ...
        schrift = tk.Label(text="Here you can calculate the profitability of a PV on your roof!",
        ...
        ausricht = tk.Label(root, text="Enter roof position", font=("Helvetica", fontsize), bg=background,
                            fg=foreground)
        ausricht.place(x=x2, y=y3 + y2 * 2)
        ausricht1 = AutocompleteCombobox(root, completevalues=ausrichtung, background='white', foreground='white',
                                         width=22, state='readonly')
        ausricht1.place(x=x1, y=y1 + y2 * 2)

        neigung = tk.Label(text="Enter roof tilt, in degrees", font=("Helvetica", fontsize), bg=background,
                           fg=foreground)
        ...
        usage = tk.Label(text="Enter yearly energy use, in kWh", font=("Helvetica", fontsize), bg=background,
                         fg=foreground)
        ...
        result = tk.Label(text="Energy generation, per Year", font=("Helvetica", fontsize), bg=background,
                          fg=foreground)
        result.place(x=x2, y=y3 + y2 * 9)
        result1 = tk.StringVar()  # plz_choice1
        result1 = tk.Entry(textvariable=result1, background='white', foreground='yellow', width=24, bg=background2,
                           fg=foreground2)
        result1.place(x=x1, y=y1 + y2 * 9)

        result2 = tk.Label(text="Energy generation, per Year", font=("Helvetica", fontsize), bg=background,
                           fg=foreground)
        result2.place(x=x2, y=y3 + y2 * 10)
        result3 = tk.StringVar()  # plz_choice1
        result3 = tk.Entry(textvariable=result3, background='white', foreground='yellow', width=24, bg=background2,
                           fg=foreground2)
        result3.place(x=x1, y=y1 + y2 * 10)

        result4 = tk.Label(text="Total 20-year cost without solar (-)", font=("Helvetica", fontsize), bg=background,
                           fg=foreground)
        result4.place(x=x2, y=y3 + y2 * 13.5)
        result5 = tk.StringVar()  # plz_choice1
        result5 = tk.Entry(textvariable=result5, background='white', foreground='orange', width=24, bg=background2,
                           fg=foreground2)
        result5.place(x=x1, y=y1 + y2 * 13.5)

        result6 = tk.Label(text="Total 20-year return for prod. energy (+)", font=("Helvetica", fontsize),
                           bg=background, fg=foreground)
        result6.place(x=x2, y=y3 + y2 * 11.5)
        result7 = tk.StringVar()  # plz_choice1
        result7 = tk.Entry(textvariable=result7, background='white', foreground='green', width=24, bg=background2,
                           fg=foreground2)
        result7.place(x=x1, y=y1 + y2 * 11.5)

        result66 = tk.Label(text="Total 20-year savings for own use (+)", font=("Helvetica", fontsize), bg=background,
                            fg=foreground)
        result66.place(x=x2, y=y3 + y2 * 12.5)
        result77 = tk.StringVar()  # plz_choice1
        result77 = tk.Entry(textvariable=result77, background='white', foreground='green', width=24, bg=background2,
                            fg=foreground2)
        result77.place(x=x1, y=y1 + y2 * 12.5)

        result8 = tk.Label(text="Total return, excl. installation costs (=)", font=("Helvetica", fontsize, 'bold'),
                           bg=background, fg=foreground)
        result8.place(x=x2, y=y3 + y2 * 14.7)
        result9 = tk.StringVar()  # plz_choice1
        result9 = tk.Entry(textvariable=result9, background='white', foreground='yellow', width=24, bg=background2,
                           fg=foreground2)
...
def knearest(LAND_PLZ, PLZ, LAND):
    ...
    if len(data1) < 1:
        data2 = data2.append({
            'LAND': LAND, 'PLZ': str(PLZ), 'STADT': None, 'BUNDESLAND': None, 'LAT': None, 'LNG': None,
            'LAND_PLZ': LAND_PLZ, 'NUTS3': None, 'NUTS2': None,
            'NUTS1': None, 'NAME': None, 'CCODE': None, 'NETT_KAP': None, 'TOTAL_PROD': None, 'INSTALLIERT': None,
            'STADT_LEVEL': None,
...
def correl1(europe, LAND_PLZ, LAND):
    ...
    sns.scatterplot(x='LNG', y='LAT', s=4.5, data=europe, palette=pal, hue='kwh/kwp_real', cmap='Blues', vmin=2000,
                    vmax=5500)
    location = europe.loc[europe['LAND_PLZ'] == LAND_PLZ]
    sns.scatterplot(x='LNG', y='LAT', s=20, data=location, palette=pal, hue='kwh/kwp_real', cmap='Blues', vmin=2000,
                    vmax=5500)
    plt.show()
...
def german_dataset(laender, LAND):
    ...
    for LAND in laender:
        if LAND == 'EL':
            ...
            latlong.rename(
                columns={0: 'LAND', 1: 'PLZ', 2: 'STADT', 3: 'BUNDESLAND', 8: 'VORWAHL', 9: 'LAT', 10: 'LNG'},
                inplace=True)
    ...
    del country_codes['alpha-3'], country_codes['iso_3166-2'], country_codes['region'], country_codes['sub-region'], \
    country_codes['intermediate-region'], country_codes['region-code'], country_codes['sub-region-code'], country_codes[
        'intermediate-region-code']
    df = pd.merge(df1, country_codes, on='LAND', how='left')

    net_installed_capacity_of_electric_power_plants_public_solar = pd.read_csv(
        'csv_data/energy_data/UNdata_Export_20221113_215209962.txt', delimiter=';')
    del net_installed_capacity_of_electric_power_plants_public_solar['Commodity - Transaction'], \
    net_installed_capacity_of_electric_power_plants_public_solar['Year'], \
    net_installed_capacity_of_electric_power_plants_public_solar['Quantity Footnotes'], \
    net_installed_capacity_of_electric_power_plants_public_solar['Unit']
    ...
    del total_solar['Commodity - Transaction'], total_solar['Year'], total_solar['Quantity Footnotes'], total_solar[
        'Unit']
    ...
    del emissions['DATAFLOW'], emissions['LAST UPDATE'], emissions['freq'], emissions['unit'], emissions['OBS_FLAG'], \
    emissions['TIME_PERIOD']
    ...
    del prices['DATAFLOW'], prices['LAST UPDATE'], prices['freq'], prices['product'], prices['currency'], prices[
        'indic_en'], prices['unit'], prices['TIME_PERIOD'], prices['OBS_FLAG']
    ...
    del dichte['DATAFLOW'], dichte['LAST UPDATE'], dichte['freq'], dichte['unit'], dichte['TIME_PERIOD'], dichte[
        'OBS_FLAG']
    ...
    del solar_prod['TIME'], solar_prod['SIEC'], solar_prod['UNIT'], solar_prod['Flag and Footnotes'], solar_prod[
        'NRG_BAL']
...
def german_dataset2(german_data, PLZ, LAND_PLZ):
    ...
    df1 = df1.reset_index()
    df1.rename(columns={'FR42': 'FRF1', 'FR61': 'FRI1', 'FR72': 'FRK1', 'FR25': 'FRD1', 'FR26': 'FRC1', 'FR52': 'FRH0',
                        'FR24': 'FRB0', 'FR21': 'FRF2', 'FR83': 'FRM0', 'FR43': 'FRC2', 'FR23': 'FRD2', 'FR10': 'FR10',
                        'FR81': 'FRJ1', 'FR63': 'FRI2', 'FR41': 'FRF3', 'FR62': 'FRJ2', 'FR30': 'FRE1', 'FR51': 'FRG0',
                        'FR22': 'FRE2', 'FR53': 'FRI3', 'FR82': 'FRL0', 'FR71': 'FRK2'}, inplace=True)
...
def correl2(german_data2):
    ...
    pal = sns.color_palette('viridis', n_colors=10, as_cmap=True)
    sns.scatterplot(x='LNG', y='LAT', s=3, data=german_data2, palette=pal, hue='SUN1', cmap='Blues', vmin=2000,
                    vmax=5500)
    plt.show()
