...
def main():
    ...
    df = pd.read_csv('lr2.csv', delimiter=';')
    del df['Einmalzahlung'], df['Rente'], df['PLZ'], df['Geschlecht2'], df['Baujahr'], df['Grundstuecksgroesse']
    ...
    dataset = df_preprocessing(df, test_length)
    df_lastrow = dataset.tail(test_length)
    dataset.drop(dataset.tail(test_length).index, inplace=True)
    with open('dataset.textmate', 'w') as file:
    ...
    i = int(traininglevel * len(dataset))
    ...
    for x in range(0, length):
        ...
        with open('df_gesamt.textmate', 'w') as file:
            file.write(str(data) + '\n')
        results = predictions(data, i)
        with open('df_results.textmate', 'w') as file:
            file.write(str(results) + '\n')

        results2 = predictions2(data,X_test2)
    ...
    roccurve = roc_curve(results)
    print("Der Lead besitzt eine Abschlusswahrscheinlichkeit von:", (float("{:.3f}".format(results2['xgb2'][0])))*100, str("%"))


def df_preprocessing(df, test_length):

    df = df.append({'Status':0,'Herkunft':float(input("Herkunft (?=nan; KOP=1; Eigenakquise=2):")),'Kanal':(input("Kanal (Unbekannt; Internet; Flyer; Andere...):")),'Geschlecht':float(input("Geschlecht (?=nan; Frau=1; Herr=2):")),'Alter':float(input("Alter (?=nan; in Jahren):")),
                            'Kinder':float(input("Kinder (?=nan; Nein=0; Ja=1):")),'Familienstand':float(input("Familienstand (?=nan; Verheiratet=1; Geschieden/Getrennt=2; Verwitwet=3; Ledig=4):")),'Alter2':float(input("Partner (?=nan; Nein=0; Ja=1):")),
                            'Bundesland':(input("Bundesland (Unbekannt; Hessen; Berlin; Bayern...):")),'Objektart':(input("Objektart (?=nan; Einfamilienhaus; Doppelhaushälfte...):")),'Keller':float(input("Keller (?=nan; Nicht unterkellert=1; Teilunterkellert=2; Unterkellert=3):")),
                            'Objektzustand':float(input("Objektzustand (?=nan; Sanierobjekt=1; Befriedigend=2; Mittel=3; Gut=4):")),'Wohnlage':float(input("Wohnlage (?=nan; Schlecht=1; Mittel=2; Gut=3):")),'Wohnhaft':float(input("Wohnhaft (?=nan; Jahreszahl):")),
                            'Heizung':float(input("Heizung (?=nan; Jahreszahl):")),'Wohnflaeche':float(input("Wohnflaeche (?=nan; In Meter^2):")),'Garagen':float(input("Garagen (?=nan; Nein=0; Ja=1):")),
                            'Stellplätze':float(input("Stellplätze (?=nan; Nein=0; Ja=1):")),'Immobilienwert_intern':float(input("Immobilienwert_intern (?=nan; Höhe):")),'Gutacherwert':float(input("Gutacherwert (?=nan; Höhe):"))}, ignore_index=True)
    ...
    # Alter
    df_lastrow = df.tail(test_length)
    df.drop(df.tail(test_length).index,inplace=True)
    df_lastrow['Alter'] = df_lastrow['Alter'].fillna(77.330938)
    ...
    # Kanal
    df1 = df1.append(df_lastrow, ignore_index=True)
    ...
    # Familienstand
    df_lastrow = df1.tail(test_length)
    df1.drop(df1.tail(test_length).index, inplace=True)
    df_lastrow['Familienstand'] = df_lastrow['Familienstand'].fillna(3.103656)
    ...
    df = df.append(df_lastrow, ignore_index=True)
    ...
    df_lastrow = df.tail(test_length)
    df.drop(df.tail(test_length).index, inplace=True)
    df_lastrow['Heizung'] = df_lastrow['Heizung'].fillna(2018.200902)
    ...
    df_lastrow = df.tail(test_length)
    df.drop(df.tail(test_length).index, inplace=True)
    df_lastrow['Wohnflaeche'] = df_lastrow['Wohnflaeche'].fillna(148.245307)
    ...
    df_lastrow = df.tail(test_length)
    df.drop(df.tail(test_length).index, inplace=True)
    df_lastrow['Immobilienwert_intern'] = df_lastrow['Immobilienwert_intern'].fillna(383591.247389)
    ...
    dfx = dfx.append(df_lastrow, ignore_index=True)

    # Gutacherwert
    dfx['Gutacherwert'] = dfx['Gutacherwert'].replace(0, None)

    df_lastrow = dfx.tail(test_length)
    dfx.drop(dfx.tail(test_length).index, inplace=True)
    df_lastrow['Gutacherwert'] = df_lastrow['Gutacherwert'].fillna(354042.151595)
    ...
    dfy = dfy.append(df_lastrow, ignore_index=True)
...
def predictions(data, i):
    rlog = []; rsvc = []; rknn = []; rgaussian = []; rperception = []; rlinear_svc = []; rsgd = []; rdecision_tree = []; rrandom_forest = []; rxgb = []; rbag = []; rada = []; rxgb2 = []

    df_train = data.head(i)
    Y_train = df_train['Status']
    del df_train['Status']
    X_train = df_train
    df_test = data.tail(len(data)-i)
...
def predictions2(data, X_test2):
    rlog = []; rsvc = []; rknn = []; rgaussian = []; rperception = []; rlinear_svc = []; rsgd = []; rdecision_tree = []; rrandom_forest = []; rxgb = []; rbag = []; rada = []; rxgb2 = []

    with open('X_test2.textmate', 'w') as file:
        file.write(str(X_test2) + '\n')
...
def confusionmatrix2(results):
    y_test = results['Status']
    y_pred = results['random_forest'].astype(int)
    cnf_matrix = metrics.confusion_matrix(y_test, y_pred)
    sns.heatmap(cnf_matrix, annot=True, cmap='Blues', fmt='g')
...
def confusionmatrix3(results):
    y_test = results['Status']
    y_pred = results['ada'].astype(int)
    cnf_matrix = metrics.confusion_matrix(y_test, y_pred)
    sns.heatmap(cnf_matrix, annot=True, cmap='Blues', fmt='g')
    plt.title('Confusion matrix: ADA Classifier')
    plt.ylabel('Actual label')
    plt.xlabel('Predicted label')
    plt.show()
...
def roc_curve(results):
    y_test = results['Status']
    y_pred = results['random_forest']
    abc, abcd, _ = metrics.roc_curve(y_test, y_pred)
    auc = metrics.roc_auc_score(y_test, y_pred)

    y_test = results['Status']
    y_pred1 = results['xgb']
    abc0, abcd0, _ = metrics.roc_curve(y_test, y_pred1)
    auc0 = metrics.roc_auc_score(y_test, y_pred1)

    y_test0 = results['Status']
    y_pred0 = results['bag']
    abc1, abcd1, _ = metrics.roc_curve(y_test0, y_pred0)
    auc1 = metrics.roc_auc_score(y_test0, y_pred0)

    y_test2 = results['Status']
    y_pred2 = results['ada']
    abc2, abcd2, _ = metrics.roc_curve(y_test2, y_pred2)
    auc2 = metrics.roc_auc_score(y_test2, y_pred2)

    plt.plot(abc, abcd, label="random_forest=" + str(auc), color='darkblue')
    plt.plot(abc0, abcd0, label="xgb=" + str(auc0), color='orange')
    plt.plot(abc1, abcd1, label="bag=" + str(auc1), color='skyblue')
    plt.plot(abc2, abcd2, label="ada=" + str(auc2), color='yellow')
...
def highrated(results):
    ...
    highrated = results
    highrated = highrated.loc[highrated['xgb2'] > 0.9]
