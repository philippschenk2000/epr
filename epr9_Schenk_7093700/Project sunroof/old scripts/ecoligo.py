...
def main():
    ...
    df = pd.read_csv('/Users/psc/PycharmProjects/ecoligo_volume/test_datasets/export11.csv', delimiter=';', decimal=',')
    ...
    #dataset = dataset[250:-1]


    Trainingsvolumen = 300
    Testvolumen = 65

    del df['Date'], df['Volume']
    ...
    with open('Trainset.textmate', 'w') as file:
...
def df_preprocessing(df):
    ...
    df['Date'] = pd.DatetimeIndex(pd.date_range('28-09-21', end='28-09-22', freq='D'))#403738318U
    ...
    df = pd.concat([df['Volume'], df['month'], df['monthday'], X_catt, df['tavg'], df['tmin'], df['tmax'], df['prcp'], df['snow'], df['wdir'], df['wspd'], df['wpgt'], df['pres'], df['tsun'], df['DAX_Close'], df['DAX_V'], df['SPY_Close'], df['SPY_V'], df['Feiertag'], df['DE_Strom[€/MWh]']], axis=1)
...
def predictions(data, i, X_testt, df_statistics, df_statistics0, date, name):
    ...
    Datum = pd.DataFrame()
    Y_train = df_train['Volume'].astype(float)
    del df_train['Volume']
    ...
    with open('Infos Trainset.textmate', 'w') as file:
